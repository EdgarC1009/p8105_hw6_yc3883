...
Fit the model. We have two possible models for candidates. model_1 uses "bhead" and "blength" as predictors. model_2 uses "bhead" and "gaweeks" as predictors. We will compare the cross-validated prediction error of these models. Also, we can also see the p-value for different variables and compare the strength of association between predictors and baby birthweight
...
The distribution of estimated log(β0∗β1) has a mean of `r mean(pull(log_est, log_b0b1))`. The standard deviation of the distribution is `r sd(pull(log_est, log_b0b1))`. This distribution is may be more approach normal distribution compared with the estimated r^2 distribution, which has a little bit shift to right. 
